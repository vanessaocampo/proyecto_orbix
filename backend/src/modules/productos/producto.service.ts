...
type CreateProductoInput =
  Pick<Producto, 'nombre' | 'precio' | 'idCategoria'> &
  Partial<
    Pick<
      Producto,
      | 'descripcion'
...
type UpdateProductoInput = Partial<
  Pick<
    Producto,
    | 'nombre'
    | 'precio'
...
async function ensureCategoriaExists(idCategoria: string) {
  ...
  if (!categoria) {
    throw ApiError.badRequest(
      `La categoría ${idCategoria} no existe`,
    )
  }
}
